...
if (fs.existsSync(OLD_DB_PATH)) {
  if (!fs.existsSync(DB_PATH)) {
    console.log('🔄 Auto-detected pre-existing cobroo.db database. Using it to preserve live data!');
    DB_PATH = OLD_DB_PATH;
  } else {
    // If both exist, compare size as a proxy of which has more real user content
    const oldSize = fs.statSync(OLD_DB_PATH).size;
    const newSize = fs.statSync(DB_PATH).size;
    if (oldSize > newSize) {
      console.log(`🔄 cobroo.db (${oldSize} bytes) is larger than cobbra.db (${newSize} bytes). Fallback to cobroo.db to preserve live data!`);
      DB_PATH = OLD_DB_PATH;
    }
  }
}
...
function getDb() {
  if (!db) {
    ...
    addColumnSafe('users', 'onboarding_answers', 'TEXT DEFAULT NULL');
    ...
    // Add reminder_date to maintenance_records safely
    addColumnSafe('maintenance_records', 'reminder_date', 'TEXT');

    // Detran / Plate columns on vehicles table safely
    addColumnSafe('vehicles', 'ipva_status', "TEXT DEFAULT 'PAGO'");
    addColumnSafe('vehicles', 'licensing_status', "TEXT DEFAULT 'EM DIA'");
    addColumnSafe('vehicles', 'licensing_expiration', "TEXT");

    // Asaas customer integration safely
    addColumnSafe('clients', 'asaas_customer_id', 'TEXT');
    addColumnSafe('clients', 'wallet_balance', 'REAL DEFAULT 0');
    addColumnSafe('clients', 'card_token', 'TEXT');
    addColumnSafe('clients', 'card_last_digits', 'TEXT');
    addColumnSafe('clients', 'withdrawal_count', 'INTEGER DEFAULT 0');

    // Asaas payment integration on charges safely
    addColumnSafe('charges', 'asaas_id', 'TEXT');
    addColumnSafe('charges', 'payment_link', 'TEXT');
    addColumnSafe('charges', 'pix_copy_paste', 'TEXT');

    // User level Cobbra Pay wallet balance and withdrawal count
    addColumnSafe('users', 'wallet_balance', 'REAL DEFAULT 0');
    addColumnSafe('users', 'withdrawal_count', 'INTEGER DEFAULT 0');

    // Create/Migrate bank_transfers table with client_id nullable
    try {
      const tableInfo = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='bank_transfers'").get();
      if (tableInfo) {
        if (tableInfo.sql.includes('client_id TEXT NOT NULL')) {
          console.log('🔄 Migrating bank_transfers table to make client_id nullable...');
          db.exec(`
            BEGIN TRANSACTION;
            ALTER TABLE bank_transfers RENAME TO bank_transfers_old;
            CREATE TABLE bank_transfers (
              id TEXT PRIMARY KEY,
              user_id TEXT NOT NULL,
              client_id TEXT, -- Nullable para saques centralizados
              amount REAL NOT NULL,
              fee REAL NOT NULL,
              net_amount REAL NOT NULL,
              status TEXT DEFAULT 'pending' CHECK(status IN ('pending', 'approved', 'failed', 'done')),
            ...
            INSERT INTO bank_transfers (id, user_id, client_id, amount, fee, net_amount, status, pix_key, pix_key_type, description, created_at, updated_at)
            SELECT id, user_id, client_id, amount, fee, net_amount, status, pix_key, pix_key_type, description, created_at, updated_at FROM bank_transfers_old;
            DROP TABLE bank_transfers_old;
            COMMIT;
          `);
          console.log('✅ bank_transfers table migrated successfully (client_id is now nullable)!');
        }
      } else {
        db.exec(`
        ...
        `);
        console.log('✅ bank_transfers table created successfully!');
      }
    } catch (e) {
      try { db.exec('ROLLBACK;'); } catch (_) {}
      console.error('Failed to initialize/migrate bank_transfers table:', e);
    }

    // Safety Migration for transactions table: support vehicle_id and type IN ('income', 'expense', 'refund')
    try {
      const tableInfo = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='transactions'").get();
      if (tableInfo && !tableInfo.sql.includes('vehicle_id')) {
        console.log('🔄 Migrating transactions table to add vehicle_id and support expense type...');
        db.exec(`
          BEGIN TRANSACTION;
          ...
          INSERT INTO transactions (id, user_id, charge_id, client_id, vehicle_id, amount, type, payment_method, reference, notes, created_at)
          SELECT id, user_id, charge_id, client_id, NULL, amount, type, payment_method, reference, notes, created_at FROM transactions_old;
    ...
    // Soft-delete support for LGPD compliance (Frente 4)
    addColumnSafe('clients', 'deleted_at', 'TEXT');

    // === HIGH-PERFORMANCE INDEXES (Fase 0 — Architectural Evolution) ===
    try {
      db.exec(`
        ...
        CREATE INDEX IF NOT EXISTS idx_traffic_fines_infraction_date ON traffic_fines(infraction_date);
        CREATE INDEX IF NOT EXISTS idx_vehicle_inspections_contract_id ON vehicle_inspections(contract_id);
        CREATE INDEX IF NOT EXISTS idx_clients_user_id ON clients(user_id);
        CREATE INDEX IF NOT EXISTS idx_transactions_user_id ON transactions(user_id);
        CREATE INDEX IF NOT EXISTS idx_reminders_charge_id ON reminders(charge_id);
        CREATE INDEX IF NOT EXISTS idx_daily_billing_user_status ON daily_billing(user_id, status);
      `);
      console.log('✅ High-performance indexes created/verified successfully!');
    } catch (e) {
      console.error('Failed to create indexes:', e);
    }

    // === TABELA DE AUDITORIA CADASTRAL (Frente 15) ===
    try {
      db.exec(`
      ...
      `);
      console.log('✅ Cadastral audit log table created/verified successfully!');
    } catch (e) {
      console.error('Failed to create cadastral_audit_log table:', e);
    }

    // === TABELA DE FILA DE WHATSAPP (Frente 6) ===
    try {
      db.exec(`
        ...
        CREATE INDEX IF NOT EXISTS idx_whatsapp_queue_status_next ON whatsapp_queue(status, next_attempt_at);
      `);
      console.log('✅ WhatsApp queue table and index created/verified successfully!');
    } catch (e) {
      console.error('Failed to create whatsapp_queue table:', e);
    }

    // === TABELAS DE AUTOMATIZAÇÃO DE LEADS & OUTBOUND ATIVO ===
    try {
      db.exec(`
        CREATE TABLE IF NOT EXISTS leads_search_queue (
          id TEXT PRIMARY KEY,
          niche TEXT NOT NULL,
          city TEXT NOT NULL,
      ...
      `);
      console.log('✅ Leads automation tables created/verified successfully!');
    } catch (e) {
      console.error('Failed to create leads automation tables:', e);
    }

    // Create client_documents table for client files/PDFs indexing
    try {
      db.exec(`
      ...
      `);
      console.log('✅ Client documents table created/verified successfully!');
    } catch (e) {
      console.error('Failed to create client_documents table:', e);
    }

    // Create blog_posts table for daily automated news/blog content
    try {
      db.exec(`
      ...
      `);
      console.log('✅ Blog posts table created/verified successfully!');
      ...
      if (postsCount === 0) {
        ...
        const initialPosts = [
          {
            id: 'post-1',
            title: 'Como cobrar um cliente atrasado sem perder a amizade?',
            slug: 'como-cobrar-um-cliente-atrasado-sem-perder-a-amizade',
            category: 'Educação',
            keywords: 'cobrança, cliente atrasado, whatsapp gentil, relacionamento comercial',
            excerpt: 'Cobrar um saldo pendente é uma das tarefas mais desconfortáveis para prestadores de serviços independentes. No entanto, a inadimplência corrói o caixa do negócio. Abordamos dicas práticas de escrita gentil e como usar o WhatsApp sem estresse.',
            content: `<h3>A Arte de Cobrar sem Perder o Cliente</h3>
<p>Cobrar um saldo pendente é uma das tarefas mais desconfortáveis para prestadores de serviços independentes. No entanto, a inadimplência corrói o caixa do negócio. Abordamos dicas práticas de escrita gentil, o uso correto de gatilhos mentais de reciprocidade e como programar lembretes automáticos no WhatsApp que mantém a parceria comercial saudável.</p>
<h4>1. O Tom da Abordagem Comercial</h4>
<p>O maior erro na hora de cobrar é o tom acusatório. Em vez de assumir que o cliente agiu de má fé, assuma o esquecimento. Frases como <i>"Passando apenas para verificar se deu tudo certo com a fatura enviada semana passada"</i> têm uma conversão muito maior e evitam ruídos de comunicação.</p>
<h4>2. O Uso de Lembretes Inteligentes</h4>
<p>Ter uma ferramenta automática faz o papel de "cobrador" por você, desvinculando sua imagem pessoal da cobrança. Com a Catarina AI do Cobbra, a régua de cobrança envia o lembrete de forma profissional pelo WhatsApp, deixando você focado apenas na entrega técnica de seus serviços.</p>
<h4>3. Facilite o Pagamento</h4>
<p>Sempre anexe o código Pix Copia e Cola e o link de fatura. Quanto menos cliques o cliente precisar dar para efetuar a transferência, mais rápida será a liquidação da dívida.</p>`
          },
          {
            id: 'post-2',
            title: 'O Guia do Pix Comercial: Maximizando o faturamento instantâneo',
            slug: 'o-guia-do-pix-comercial-maximizando-o-faturamento-instantaneo',
            category: 'Finanças',
            keywords: 'pix comercial, taxa zero, faturamento instantâneo, conciliação de caixa',
            excerpt: 'O Pix revolucionou a circulação de caixa em todo o país. Contudo, muitos freelancers ainda perdem dinheiro com taxas indevidas ou dificuldades no envio manual do copia e cola. Explicamos como automatizar chaves de faturamento e integrar checkouts.',
            content: `<h3>Maximizando os Ganhos com o Pix Comercial</h3>
<p>O Pix revolucionou a circulação de caixa em todo o país. Contudo, muitos freelancers e microempresários ainda perdem dinheiro com taxas indevidas de contas PJ tradicionais ou dificuldades no envio do "Copia e Cola" manual.</p>
<h4>Por que Automatizar Cobranças Pix?</h4>
<p>O envio manual de chaves Pix gera atrito. O cliente precisa abrir o app do banco, copiar a chave, digitar o valor e conferir os dados. Um único dígito errado pode cancelar a transação ou enviar o valor para a pessoa errada. Com cobranças Pix dinâmicas (contendo o valor e vencimento pré-definidos), o cliente apenas abre o app do banco, lê o QR Code e autoriza o pagamento imediato.</p>
<h4>Como Evitar Taxas no Pix PJ</h4>
<p>Grandes instituições financeiras cobram tarifas de até 1.99% por cada Pix recebido em contas PJ. Plataformas como o Cobbra integram diretamente a sua carteira digital para processar transações 100% livres de intermediações, garantindo que o valor integral caia direto no seu saldo business na hora.</p>`
          },
          {
            id: 'post-3',
            title: '5 planilhas essenciais de fluxo de caixa que todo MEI precisa',
            slug: '5-planilhas-essenciais-de-fluxo-de-caixa-que-todo-mei-precisa',
            category: 'Administração',
            keywords: 'fluxo de caixa, planilhas gratuitas, controle financeiro mei, gestão de caixa',
            excerpt: 'Rastrear as contas de entrada e saída é vital para evitar surpresas financeiras no fechamento do mês. Apresentamos modelos descomplicados de controle de caixa, conciliação Pix e controle de frotas e cauções.',
            content: `<h3>Organize Suas Finanças com Modelos de Caixa Simples</h3>
<p>Rastrear as contas de entrada e saída é vital para evitar surpresas financeiras no fechamento do mês. Apresentamos os 5 tipos de planilhas de controle financeiro fundamentais para o Microempreendedor Individual (MEI) e autônomos.</p>
<h4>1. Planilha de Fluxo de Caixa Diário</h4>
<p>Serve para registrar todas as entradas e saídas físicas de dinheiro do dia a dia. É o feijão com arroz da contabilidade.</p>
<h4>2. Controle de Contas a Receber</h4>
<p>Essencial para rastrear parcelamentos, carnês ou mensalidades ativas de clientes. Mostra quem já pagou e quem está inadimplente.</p>
<h4>3. Planilha de Custos Fixos vs Variáveis</h4>
<p>Ajuda a identificar onde estão os gargalos do negócio e qual o ponto de equilíbrio necessário para cobrir os custos operacionais básicos.</p>
<h4>4. Conciliação de Recebíveis Pix</h4>
<p>Importante para bater os comprovantes enviados pelos clientes com o extrato real da conta bancária, evitando fraudes de falsos Pix.</p>
<h4>5. Controle de Frotas e Cauções (para Frotistas)</h4>
<p>Desenvolvida para quem aluga veículos ou equipamentos e precisa manter controle dos valores retidos a título de garantia.</p>`
          }
        ];

        const stmt = db.prepare(`
          INSERT INTO blog_posts (id, title, slug, excerpt, content, category, keywords)
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `);
        for (const post of initialPosts) {
          stmt.run(post.id, post.title, post.slug, post.excerpt, post.content, post.category, post.keywords);
        }
    ...
    // Create capital_custody table
    try {
      db.exec(`
        CREATE TABLE IF NOT EXISTS capital_custody (
          id TEXT PRIMARY KEY,
          user_id TEXT NOT NULL,
          client_id TEXT NOT NULL,
          ...
          billing_frequency TEXT DEFAULT 'daily' CHECK(billing_frequency IN ('daily', 'weekly', 'monthly')),
          collateral_info TEXT,
          custom_message_template TEXT,
          late_interest_rate REAL DEFAULT 0.01,
        ...
        CREATE INDEX IF NOT EXISTS idx_capital_custody_user ON capital_custody(user_id);
        CREATE INDEX IF NOT EXISTS idx_capital_custody_client ON capital_custody(client_id);
      `);
      console.log('✅ Capital custody tables created/verified successfully!');
    } catch (e) {
      console.error('Failed to create capital_custody table:', e);
    }

    // Create capital_custody_history table
    try {
      db.exec(`
        CREATE TABLE IF NOT EXISTS capital_custody_history (
          id TEXT PRIMARY KEY,
          custody_id TEXT NOT NULL,
          type TEXT NOT NULL CHECK(type IN ('lease_start', 'daily_fee', 'amortization', 'repayment', 'fee_payment')),
          amount REAL NOT NULL,
          charge_id TEXT,
          notes TEXT,
          created_at TEXT DEFAULT (datetime('now')),
          FOREIGN KEY (custody_id) REFERENCES capital_custody(id) ON DELETE CASCADE
        );
        CREATE INDEX IF NOT EXISTS idx_capital_custody_hist_custody ON capital_custody_history(custody_id);
      `);
      console.log('✅ Capital custody history tables created/verified successfully!');
    } catch (e) {
      console.error('Failed to create capital_custody_history table:', e);
    }

    // Migration of avatar_url to clients and users
    addColumnSafe('clients', 'avatar_url', 'TEXT');
    addColumnSafe('users', 'avatar_url', 'TEXT');
    addColumnSafe('capital_custody', 'late_interest_rate', 'REAL DEFAULT 0.01');
...
function initializeDatabase() {
  try {
    ...
    // Ensure admin@cobbra.com.br exists and has password 'admin' in production
    try {
      const bcrypt = require('bcryptjs');
      const passwordHash = bcrypt.hashSync('admin', 12);
      
      const adminExists = db.prepare("SELECT id FROM users WHERE email = ?").get('admin@cobbra.com.br');
      if (!adminExists) {
        console.log('🚀 admin@cobbra.com.br not found. Seeding admin user...');
        ...
        `).run(
          ...
          'email',
          'Cobbra Admin Pay',
          'Painel de Controle e Gestão Financeira',
          'enterprise',
        ...
        console.log('✅ admin@cobbra.com.br seeded successfully!');
      } else if (process.env.NODE_ENV !== 'production') {
        // Force update admin password to 'admin' ONLY in non-production environments for recovery/local testing
        db.prepare("UPDATE users SET password_hash = ?, role = 'admin_senior' WHERE email = ?").run(passwordHash, 'admin@cobbra.com.br');
        console.log('✅ admin@cobbra.com.br password force-reset to "admin" on startup (Non-prod recovery mode).');
      }
    } catch (e) {
    ...
    // === AUTO-IMPORT OUTBOUND LEADS FROM JSON DUMP ===
    try {
      const dumpPath = path.join(process.cwd(), 'public', 'leads_dump.json');
      if (fs.existsSync(dumpPath)) {
        const fileContent = fs.readFileSync(dumpPath, 'utf8');
        const dumpLeads = JSON.parse(fileContent);
        if (Array.isArray(dumpLeads) && dumpLeads.length > 0) {
          const currentCount = db.prepare("SELECT COUNT(*) as count FROM leads_prospects").get().count;
          if (currentCount < 400) {
            console.log(`🚀 Found leads_dump.json with ${dumpLeads.length} leads. Importing into database...`);
            ...
            db.transaction(() => {
              ...
              // Force reset all 'sent' leads back to 'ready_to_send' to let the user test them fresh!
              db.prepare("UPDATE leads_prospects SET status = 'ready_to_send' WHERE status = 'sent'").run();
            })();
            console.log(`✅ Loaded ${dumpLeads.length} prospects safely into production database and reset sent state!`);
          }
        }
      }
    } catch (dumpErr) {
      console.error('❌ Failed to auto-import leads from dump:', dumpErr.message);
    }
