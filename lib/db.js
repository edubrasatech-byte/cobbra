...
function initializeDatabase() {
  try {
    ...
    // Force reset all sent/failed leads to ready_to_send on startup to support testing fresh chips
    try {
      db.prepare("UPDATE leads_prospects SET status = 'ready_to_send', attempts = 0, sent_at = NULL WHERE status = 'sent' OR status = 'failed'").run();
      console.log('🔄 Force-reset all sent/failed leads in production database to ready_to_send on boot!');
    } catch (_) {}
