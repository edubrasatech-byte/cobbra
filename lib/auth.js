...
function getUserFromRequest(request) {
  ...
  const user = queryOne('SELECT id, name, email, role, phone, pix_key, pix_key_type, business_name, business_description, business_niche, collection_rigor, plan, plan_expires_at, status, onboarding_completed, score_limit_good, score_limit_regular, avatar_url, created_at FROM users WHERE id = ?', [decoded.id]);
  return user || null;
}
...
// Gerar token de impersonação com expiração estrita de 1 hora (Frente 4 — Hardening)
function generateImpersonateToken(adminUser, targetUserId) {
  return jwt.sign(
    {
    ...
    {
      expiresIn: '1h',
      issuer: 'cobbra-impersonate'
    }
  );
}

// Verificar token de impersonação com issuer separado para isolamento de segurança
