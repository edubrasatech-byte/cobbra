...
function getUserFromRequest(request) {
  ...
  const user = queryOne('SELECT id, name, email, role, phone, pix_key, pix_key_type, business_name, business_description, plan, plan_expires_at, status, onboarding_completed, score_limit_good, score_limit_regular, created_at FROM users WHERE id = ?', [decoded.id]);
  return user || null;
}
...
async function registerUser({ name, email, password, phone }) {
  ...
  run(
    `INSERT INTO users (id, name, email, password_hash, phone, role, plan, plan_expires_at)
     VALUES (?, ?, ?, ?, ?, 'user', 'trial', ?)`,
    [id, name, email, password_hash, phone || null, trialExpires.toISOString()]
  );
